...
Personal_Sub2 は、公式 Sub2API `v0.1.168` を基に個人で開発し、独立して保守しているバージョンです。この統合リリースは `v0.1.168-P1` です。
...
- **Passkey 認証**：プロフィール画面で Passkey を登録・管理し、パスワードレスでログインできます。管理者はシステム設定で有効化を制御でき、登録と取り消しにはアカウントパスワードの確認が必要です。
- **モデル広場**：グループ単位のモデル料金を公開ページで確認でき、管理者が表示範囲を設定できます。
- **Kimi K3 対応**：課金および思考プロトコルに対応し、`1M` コンテキストサフィックスを正しく認識します。
- **管理とデプロイ**：アカウントのモデル許可リストからモデル ID をコピーでき、`SKIP_SETUP` で初期セットアップを明示的にスキップできます。
- **同時更新の安全性**：ユーザーと API キーは宣言された列だけを更新し、同時操作で無関係な項目が上書きされることを防ぎます。
- **信頼性とセキュリティ修正**：セキュリティ監査設定の復号失敗による消失と保存不能を修正し、Claude OAuth の system キャッシュ境界、Codex の Web 検索ツール宣言、GPT-5.6 の `max` 推論強度を保持するとともに、ストレージ障害時の OpenAI Live セッション精算を強化しました。
