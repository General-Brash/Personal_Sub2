...
Personal_Sub2 は [Sub2API](https://github.com/Wei-Shaw/sub2api) を基盤として保守されている個人フォークです。上流のゲートウェイ機能を維持しつつ、このデプロイ向けの拡張機能を独立して保守しています。本プロジェクトは上流 Sub2API の公式プロジェクトではなく、上流のメンテナーを代表したり、上流を装ったりするものではありません。

上流 Sub2API と比較して、このフォークには現在次の機能が追加されています：

- **一時クレジット** - 恒久残高とは別に、有効期限付き一時クレジットの付与、消費、利用可能額を記録します。
- **毎日のチェックイン** - 設定可能なルールに従って一時クレジットを付与し、チェックイン状況と履歴をユーザーに提供します。
- **銀行** - 一時クレジットの前借りと、恒久残高から一時クレジットへの交換をサポートし、設定可能な上限、精算ルール、台帳記録を提供します。
- **セキュリティ監査の二次レビュー** - ASCII キーワードの境界照合を改善し、ヒットしたテキストを独立した `intent-classifier` サービスで二次判定できます。`off`、`shadow`、`enforce` モードに加え、バージョン管理された ONNX モデルパッケージ、ヘルス／レディネス表示、検証、アクティベーション、ロールバックをサポートします。

本リポジトリには本番用モデルの重みは含まれていません。`shadow` または `enforce` でモデル判定を利用する前に、運用者は [`MODEL_PACKAGE.md`](services/intent-classifier/MODEL_PACKAGE.md) に従ったモデルパッケージを用意し、アクティベートする必要があります。
