...
Personal_Sub2 is a personal fork built on [Sub2API](https://github.com/Wei-Shaw/sub2api). It retains the upstream gateway foundation and adds independently maintained features for this deployment. It is not the official upstream Sub2API project and does not represent or impersonate the upstream maintainers.

Compared with upstream Sub2API, this fork currently adds:

- **Temporary credit** - Tracks expiring temporary-credit grants, consumption, and available credit alongside the permanent balance.
- **Daily check-in** - Issues configurable daily rewards as temporary credit and exposes check-in status and history to users.
...
- **Security-audit secondary review** - Improved ASCII keyword-boundary matching can trigger an independent `intent-classifier` service for a second decision. The integration supports `off`, `shadow`, and `enforce` modes, versioned ONNX model packages, health/readiness reporting, validation, activation, and rollback.

Production model weights are not included. Operators must provide and activate a model package that follows [`MODEL_PACKAGE.md`](services/intent-classifier/MODEL_PACKAGE.md) before model-backed review can be used in `shadow` or `enforce` mode.
