...
type UsageLogQuery struct {
	...
	withTemporaryCreditConsumptions *TemporaryCreditConsumptionQuery
...
// QueryTemporaryCreditConsumptions chains the current query on the "temporary_credit_consumptions" edge.
func (_q *UsageLogQuery) QueryTemporaryCreditConsumptions() *TemporaryCreditConsumptionQuery {
...
func (_q *UsageLogQuery) Clone() *UsageLogQuery {
	...
	return &UsageLogQuery{
		...
		withTemporaryCreditConsumptions: _q.withTemporaryCreditConsumptions.Clone(),
...
// WithTemporaryCreditConsumptions tells the query-builder to eager-load the nodes that are connected to
// the "temporary_credit_consumptions" edge. The optional arguments are used to configure the query builder of the edge.
func (_q *UsageLogQuery) WithTemporaryCreditConsumptions(opts ...func(*TemporaryCreditConsumptionQuery)) *UsageLogQuery {
	query := (&TemporaryCreditConsumptionClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	_q.withTemporaryCreditConsumptions = query
	return _q
}
...
func (_q *UsageLogQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*UsageLog, error) {
	var (
		...
		loadedTypes = [6]bool{
			...
			_q.withTemporaryCreditConsumptions != nil,
		}
	)
	...
	if query := _q.withTemporaryCreditConsumptions; query != nil {
		if err := _q.loadTemporaryCreditConsumptions(ctx, query, nodes,
			func(n *UsageLog) { n.Edges.TemporaryCreditConsumptions = []*TemporaryCreditConsumption{} },
			func(n *UsageLog, e *TemporaryCreditConsumption) {
				n.Edges.TemporaryCreditConsumptions = append(n.Edges.TemporaryCreditConsumptions, e)
			}); err != nil {
			return nil, err
		}
	}
...
func (_q *UsageLogQuery) loadTemporaryCreditConsumptions(ctx context.Context, query *TemporaryCreditConsumptionQuery, nodes []*UsageLog, init func(*UsageLog), assign func(*UsageLog, *TemporaryCreditConsumption)) error {
	...
	if len(query.ctx.Fields) > 0 {
		query.ctx.AppendFieldOnce(temporarycreditconsumption.FieldUsageLogID)
	}
	query.Where(predicate.TemporaryCreditConsumption(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(usagelog.TemporaryCreditConsumptionsColumn), fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
