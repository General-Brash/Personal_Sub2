...
func (_q *APIKeyQuery) QueryBatchImageCreditHolds() *BatchImageCreditHoldQuery {
...
func (_q *APIKeyQuery) WithBatchImageCreditHolds(opts ...func(*BatchImageCreditHoldQuery)) *APIKeyQuery {
	query := (&BatchImageCreditHoldClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (_q *APIKeyQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*APIKey, error) {
	...
	if query := _q.withBatchImageCreditHolds; query != nil {
		if err := _q.loadBatchImageCreditHolds(ctx, query, nodes,
			func(n *APIKey) { n.Edges.BatchImageCreditHolds = []*BatchImageCreditHold{} },
			func(n *APIKey, e *BatchImageCreditHold) {
...
func (_q *APIKeyQuery) loadBatchImageCreditHolds(ctx context.Context, query *BatchImageCreditHoldQuery, nodes []*APIKey, init func(*APIKey), assign func(*APIKey, *BatchImageCreditHold)) error {
	fks := make([]driver.Value, 0, len(nodes))
	nodeids := make(map[int64]*APIKey)
	for i := range nodes {
		fks = append(fks, nodes[i].ID)
	...
	if len(query.ctx.Fields) > 0 {
		query.ctx.AppendFieldOnce(batchimagecredithold.FieldAPIKeyID)
	}
	query.Where(predicate.BatchImageCreditHold(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(apikey.BatchImageCreditHoldsColumn), fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
