...
func TestPricingRequestToService_ChannelMultipliersStayOutOfAccountStats(t *testing.T) {
	fast, flex, max, cache1h := 2.0, 0.5, 1.75, 0.003
	inputMultiplier := 1.25
	req := channelModelPricingRequest{Models: []string{"gpt-6-astra"}, FastMultiplier: &fast, FlexMultiplier: &flex, MaxReasoningEffortMultiplier: &max, CacheWrite1hPrice: &cache1h, Intervals: []pricingIntervalRequest{{InputMultiplier: &inputMultiplier, CacheWrite1hPrice: &cache1h}}}
	main := pricingRequestToService([]channelModelPricingRequest{req}, true)
	require.Equal(t, &fast, main[0].FastMultiplier)
	require.Equal(t, &flex, main[0].FlexMultiplier)
	require.Equal(t, &max, main[0].MaxReasoningEffortMultiplier)
	require.Equal(t, &inputMultiplier, main[0].Intervals[0].InputMultiplier)
	stats := accountStatsPricingRuleRequestToService(accountStatsPricingRuleRequest{Pricing: []channelModelPricingRequest{req}})
	require.Nil(t, stats.Pricing[0].FastMultiplier)
	require.Nil(t, stats.Pricing[0].FlexMultiplier)
	require.Nil(t, stats.Pricing[0].MaxReasoningEffortMultiplier)
	require.Nil(t, stats.Pricing[0].Intervals[0].InputMultiplier)
	require.Equal(t, &cache1h, stats.Pricing[0].CacheWrite1hPrice)
	require.Equal(t, &cache1h, stats.Pricing[0].Intervals[0].CacheWrite1hPrice)
}
