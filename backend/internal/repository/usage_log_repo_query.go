...
func (r *usageLogRepository) ListWithFilters(ctx context.Context, params pagination.PaginationParams, filters UsageLogFilters) ([]service.UsageLog, *pagination.PaginationResult, error) {
	...
	if requestedEffort := strings.TrimSpace(filters.RequestedReasoningEffort); requestedEffort != "" {
		conditions = append(conditions, fmt.Sprintf("requested_reasoning_effort = $%d", len(args)+1))
		args = append(args, requestedEffort)
	}
...
// appendNativeCompactionV2WhereCondition adds an exact boolean filter while
// keeping callers that query an aliased usage_logs relation source-compatible.
func appendNativeCompactionV2WhereCondition(conditions []string, args []any, enabled *bool, tableAlias string) ([]string, []any) {
	if enabled == nil {
		return conditions, args
	}
	column := "native_compaction_v2"
	if alias := strings.TrimSpace(tableAlias); alias != "" {
		column = alias + "." + column
	}
