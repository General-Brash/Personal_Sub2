...
// TestPrepareUsageLogInsert_SessionIDArgWiring pins session_id and
// native_compaction_v2 to the arg slice / arg-type table so all INSERT column
// lists stay in sync. created_at is always last.
func TestPrepareUsageLogInsert_SessionIDArgWiring(t *testing.T) {
	require.Len(t, usageLogInsertArgTypes, 62, "arg-type table must include the full usage audit contract")
	...
	// created_at is last; native_compaction_v2 precedes it; session_id precedes that.
	...
	require.Equal(t, false, prepared.args[len(prepared.args)-2])
}
...
func TestUsageLogInsertQueries_IncludeSessionID(t *testing.T) {
	...
	require.Contains(t, bestEffortQuery, "requested_reasoning_effort")
	require.Contains(t, bestEffortQuery, "native_compaction_v2")
