...
// CanonicalGrokRuntimeModelID normalizes a Grok runtime/build alias to the
// public model family used by forwarding, scheduling, quota, and transient
// cooldown state. Empty input remains empty so callers do not invent a model.
func CanonicalGrokRuntimeModelID(model string) string {
	if strings.TrimSpace(model) == "" {
		return ""
	}
	return ResolveGrokTextResponsesModelID(model)
}
