...
func (s *OpenAIGatewayService) fetchOpenAIImageURLBase64(ctx context.Context, account *Account, rawURL string) (string, error) {
	if strings.HasPrefix(strings.ToLower(strings.TrimSpace(rawURL)), "data:") {
		data, err := decodeOpenAIImageDataURLForBackfill(rawURL)
		if err != nil {
			return "", err
		}
...
// decodeOpenAIImageDataURLForBackfill decodes an inline data URL under the same
// byte-size and content-sniffing policy as an HTTP download. The declared media
// type is intentionally not trusted.
func decodeOpenAIImageDataURLForBackfill(rawURL string) ([]byte, error) {
	rawURL = strings.TrimSpace(rawURL)
	comma := strings.IndexByte(rawURL, ',')
	if comma < 0 || !strings.HasSuffix(strings.ToLower(strings.TrimSpace(rawURL[:comma])), ";base64") {
		return nil, errors.New("data url must contain a base64 payload")
	}
	payload := strings.TrimSpace(rawURL[comma+1:])
	if payload == "" {
		return nil, errors.New("data url payload is empty")
	}
	payload = strings.TrimRight(payload, "=")
	payload += strings.Repeat("=", (4-len(payload)%4)%4)
	if int64(base64.StdEncoding.DecodedLen(len(payload))) > openAIImageMaxDownloadBytes {
		return nil, fmt.Errorf("data url image exceeds %d bytes", openAIImageMaxDownloadBytes)
	}
	...
	if !isBackfillImageContent(data) {
		return nil, errors.New("data url content is not an allowed image format")
	}
	return data, nil
}
