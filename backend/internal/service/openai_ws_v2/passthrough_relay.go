...
func parseUsageAndAccumulate(
	...
	if reasoningTokens > 0 {
		outputTokens = int(includeIndependentReasoningTokens(
...
func relayUsageHasTokens(usage Usage) bool {
	return usage.InputTokens > 0 || usage.OutputTokens > 0 || usage.CacheCreationInputTokens > 0 || usage.CacheReadInputTokens > 0 || usage.ImageOutputTokens > 0
}
...
func includeIndependentReasoningTokens(input, output, total, reasoning int64) int64 {
	if input < 0 || output < 0 || reasoning <= 0 || total <= 0 || total == input+output {
		return output
	}
