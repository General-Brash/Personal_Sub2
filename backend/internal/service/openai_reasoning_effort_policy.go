...
//
// The optional overLimit parameter keeps existing callers source-compatible:
// omitted means the historical downgrade behavior.
func WithOpenAIReasoningEffortPolicy(ctx context.Context, maxEffort string, mappings []ReasoningEffortMapping, overLimit ...string) context.Context {
	...
	overLimitAction := ReasoningEffortOverLimitDowngrade
	if len(overLimit) > 0 {
		if normalized := NormalizeMaxReasoningEffortOverLimit(overLimit[0]); normalized != "" {
			overLimitAction = normalized
		}
	}
...
// It preserves the historical downgrade-only signature. New gateway paths that
// must distinguish deny from downgrade use
// ApplyOpenAIReasoningEffortPolicyFromContextWithOverLimit.
func ApplyOpenAIReasoningEffortPolicyFromContext(ctx context.Context, body []byte) ([]byte, bool) {
	updated, changed, _ := ApplyOpenAIReasoningEffortPolicyFromContextWithOverLimit(ctx, body)
	return updated, changed
}

// ApplyOpenAIReasoningEffortPolicyFromContextWithOverLimit applies a bound
// policy and returns a typed over-limit error when the configured action is
// deny. An unbound request is returned byte-for-byte unchanged.
func ApplyOpenAIReasoningEffortPolicyFromContextWithOverLimit(ctx context.Context, body []byte) ([]byte, bool, error) {
	if ctx == nil {
	...
	return ApplyOpenAIReasoningEffortPolicyWithOverLimit(body, policy.maxEffort, policy.mappings, policy.overLimit)
}
...
// ApplyOpenAIReasoningEffortPolicy applies the historical downgrade behavior.
// New ingress paths that need deny semantics must call
// ApplyOpenAIReasoningEffortPolicyWithOverLimit.
func ApplyOpenAIReasoningEffortPolicy(body []byte, maxEffort string, mappings []ReasoningEffortMapping) ([]byte, bool) {
	updated, changed, _ := ApplyOpenAIReasoningEffortPolicyWithOverLimit(
		body,
		maxEffort,
		mappings,
		ReasoningEffortOverLimitDowngrade,
...
// ApplyOpenAIReasoningEffortPolicyWithOverLimit applies one mapping (optionally scoped to
...
func ApplyOpenAIReasoningEffortPolicyWithOverLimit(body []byte, maxEffort string, mappings []ReasoningEffortMapping, overLimit string) ([]byte, bool, error) {
	return ApplyReasoningEffortPolicy(body, maxEffort, mappings, overLimit)
}

// ApplyReasoningEffortPolicy applies the shared mapping/ceiling contract to OpenAI and Anthropic ingress.
...
func applyOpenAIWSReasoningEffortPolicy(payload []byte, hooks *OpenAIWSIngressHooks) ([]byte, error) {
	...
	capped, changed, err := ApplyOpenAIReasoningEffortPolicyWithOverLimit(payload, hooks.MaxReasoningEffort, hooks.ReasoningEffortMappings, hooks.MaxReasoningEffortOverLimit)
	if err != nil {
		return payload, err
	}
	if changed {
