...
func (a *Account) GetUpstreamModelMetadataSnapshot() *UpstreamModelMetadataSnapshot {
	...
	// Decode model entries independently. A single malformed metadata entry must
	// not hide the other capabilities already synchronized for this account.
	var envelope struct {
		Source   string                     `json:"source"`
		SyncedAt string                     `json:"synced_at"`
	...
	snapshot := UpstreamModelMetadataSnapshot{
		Source: envelope.Source, SyncedAt: envelope.SyncedAt,
		Models: make(map[string]UpstreamModelMetadata, len(envelope.Models)),
	}
	for modelID, rawMetadata := range envelope.Models {
		var metadata UpstreamModelMetadata
		if err := json.Unmarshal(rawMetadata, &metadata); err != nil {
			continue
		}
		if strings.TrimSpace(metadata.ID) == "" {
			metadata.ID = strings.TrimSpace(modelID)
		}
		snapshot.Models[modelID] = metadata
	}
...
func (s *AccountTestService) SyncUpstreamModelCatalog(ctx context.Context, account *Account) (*UpstreamModelCatalog, error) {
	...
	liveListAvailable := err == nil && len(models) > 0
	if err != nil {
...
func (s *AccountTestService) fetchUpstreamModelList(ctx context.Context, account *Account) ([]string, []byte, error) {
	...
	// An empty but valid upstream catalog is not a destructive sync. Let the
	// caller retain the previous metadata snapshot and use configured mappings
	// as the best available capability source instead of failing the whole sync.
