...
func (s *BillingService) CalculateTokenCostForRequest(req TokenCostRequest) (*CostBreakdown, error) {
	...
	cost, err := s.CalculateCostWithServiceTier(req.Model, req.Tokens, req.RateMultiplier, req.ServiceTier)
	if err == nil {
		applyCostBreakdownMultiplier(cost, maxReasoningEffortBillingMultiplier(req.Model, req.ReasoningEffort, nil))
	}
	return cost, err
}
