...
func TestCalculatePerRequestCost_AppliesResolvedTimePricing(t *testing.T) {
	unitPrice := 0.04
	channel := []ChannelModelPricing{{
		Platform: PlatformOpenAI, Models: []string{"gpt-image-2"}, BillingMode: BillingModeImage,
		PerRequestPrice: testPtrFloat64(unitPrice),
		TimePricing: &ChannelTimePricing{
			Timezone: "UTC",
			Periods:  []ChannelTimePricingPeriod{{StartTime: "09:00", EndTime: "17:00", Multiplier: 2}},
		},
	}}
	bs, resolver := newTokenCostTestEnv(t, PlatformOpenAI, channel, nil)
	group := enabledGroup(PlatformOpenAI)
	gid := group.ID
	resolved := resolver.Resolve(context.Background(), PricingInput{Model: "gpt-image-2", GroupID: &gid, Group: group})
	loc := time.UTC
	...
	for _, tc := range []struct {
		...
		{name: "inside period", at: time.Date(2026, time.June, 29, 10, 0, 0, 0, loc), want: 0.08},
		{name: "outside period", at: time.Date(2026, time.June, 29, 18, 0, 0, 0, loc), want: 0.04},
	} {
		t.Run(tc.name, func(t *testing.T) {
			cost, err := bs.CalculateCostUnified(CostInput{
				Ctx: context.Background(), Model: "gpt-image-2", Group: group,
				RequestCount: 1, RateMultiplier: 1, PricingAt: tc.at,
				Resolver: resolver, Resolved: resolved,
			})
			require.NoError(t, err)
...
func TestCalculateCostUnified_ReusesResolvedPricingSnapshot(t *testing.T) {
	group := &Group{
		ID: 100, Platform: PlatformOpenAI, LongContextPricingEnabled: true,
	...
	// Simulate a catalog/group mutation between preflight and settlement. The
	// pre-resolved request must retain the price selected during preflight.
	group.ModelPricing[0].PerRequestPrice = testPtrFloat64(0.9)
	cost, err := bs.CalculateCostUnified(CostInput{
		Ctx: context.Background(), Model: "gpt-image-2", Group: group,
