...
func (s *BillingService) GetModelPricingWithChannel(model string, channelPricing *ChannelModelPricing) (*ModelPricing, error) {
	if err := validateChannelPricingMultipliers(model, channelPricing); err != nil {
		return nil, err
	}
...
func (s *BillingService) CalculateCostUnified(input CostInput) (*CostBreakdown, error) {
	...
	if resolved == nil {
		return nil, billingPricingUnavailable(input.Model, "token")
	}
	if err := validateResolvedPricingMultipliers(input.Model, resolved); err != nil {
		return nil, err
	}
...
func (s *BillingService) calculateTokenCost(resolved *ResolvedPricing, input CostInput) (*CostBreakdown, error) {
	...
	if err := validateModelPricingMultipliers(input.Model, "token", pricing); err != nil {
		return nil, err
	}
