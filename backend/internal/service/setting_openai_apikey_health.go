...
func (s *SettingService) SetOpenAIAPIKeyHealthBreakerSettings(ctx context.Context, settings *OpenAIAPIKeyHealthBreakerSettings) error {
	...
	if settings == nil {
		return errors.New("OpenAI API key health breaker settings cannot be nil")
	}
	...
	if err := s.settingRepo.Set(ctx, SettingKeyOpenAIAPIKeyHealthBreakerSettings, string(encoded)); err != nil {
