...
type codexMetadataSyncRepoStub struct {
	AccountRepository
	updates map[string]any
}

func (r *codexMetadataSyncRepoStub) UpdateExtra(_ context.Context, _ int64, updates map[string]any) error {
	r.updates = updates
	return nil
}

// An empty-but-valid upstream /models response is not evidence that previously
// synchronized capabilities disappeared. This is the metadata-side equivalent
// of the official SyncUpstreamModelCatalog retention contract.
func TestCodexMetadataSyncEmptyUpstreamCatalogRetainsExistingCapabilities(t *testing.T) {
	upstream := &httpUpstreamRecorder{responses: []*http.Response{
		{StatusCode: 200, Header: http.Header{"Content-Type": []string{"application/json"}}, Body: io.NopCloser(strings.NewReader(`{"data":[]}`))},
		{StatusCode: 502, Header: http.Header{"Content-Type": []string{"application/json"}}, Body: io.NopCloser(strings.NewReader(`{"error":"registry unavailable"}`))},
	}}
	repo := &codexMetadataSyncRepoStub{}
	...
	account.SetUpstreamModelMetadataSnapshot(UpstreamModelMetadataSnapshot{Models: map[string]UpstreamModelMetadata{
		"mapped-model": {ID: "mapped-model", Reasoning: codexMetadataBoolPtr(true), InputModalities: []string{"text", "image"}, ContextWindow: 256_000},
	}})
	...
	require.Nil(t, repo.updates, "empty live catalog plus unavailable enrichment must not erase the previous snapshot")
	snapshot := account.GetUpstreamModelMetadataSnapshot()
	require.NotNil(t, snapshot)
	require.Equal(t, int64(256_000), snapshot.Models["mapped-model"].ContextWindow)
	require.Equal(t, []string{"text", "image"}, snapshot.Models["mapped-model"].InputModalities)
}

func codexMetadataBoolPtr(value bool) *bool { return &value }
