...
func TestFetchOpenAIImageURLBase64_DataURLUsesDownloadSafetyPolicy(t *testing.T) {
	validPNG := base64.StdEncoding.EncodeToString(b64BackfillPNGBytes)
	tests := []struct {
		name    string
		rawURL  string
		want    string
		...
		{
			name:   "declared MIME is ignored when bytes are a permitted image",
			rawURL: "data:text/html;base64," + validPNG,
			want:   validPNG,
		},
		{
			name:    "declared image MIME cannot bypass byte sniffing",
			rawURL:  "data:image/png;base64," + base64.StdEncoding.EncodeToString([]byte("plain text that is not an image")),
			wantErr: "allowed image format",
		},
		{
			name:    "non-base64 data URL is rejected",
			rawURL:  "data:image/png,not-base64",
			wantErr: "base64",
		},
		{
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			got, err := (*OpenAIGatewayService)(nil).fetchOpenAIImageURLBase64(context.Background(), nil, tt.rawURL)
			if tt.wantErr != "" {
				require.Error(t, err)
...
func TestFetchOpenAIImageURLBase64_DataURLRejectsOversizeBeforeDecode(t *testing.T) {
	payload := base64.StdEncoding.EncodeToString(make([]byte, openAIImageMaxDownloadBytes+1))
	got, err := (*OpenAIGatewayService)(nil).fetchOpenAIImageURLBase64(
		context.Background(),
		nil,
...
func TestFetchOpenAIImageURLBase64_RejectsStreamedOversizeBody(t *testing.T) {
	payload := make([]byte, openAIImageMaxDownloadBytes+1)
	copy(payload, b64BackfillPNGBytes)
	upstream := &httpUpstreamRecorder{resp: b64BackfillImageResponse(http.StatusOK, "image/png", payload)}
	svc := &OpenAIGatewayService{cfg: &config.Config{}, httpUpstream: upstream}

	got, err := svc.fetchOpenAIImageURLBase64(context.Background(), b64BackfillAccount(true), "https://cdn.example.com/a.png")
	require.Error(t, err)
	require.Contains(t, err.Error(), "exceeds")
	require.Empty(t, got)
}

func TestFetchOpenAIImageURLBase64_HonorsCanceledContext(t *testing.T) {
	upstream := &b64BackfillContextUpstream{}
	svc := &OpenAIGatewayService{cfg: &config.Config{}, httpUpstream: upstream}
	ctx, cancel := context.WithCancel(context.Background())
	cancel()

	got, err := svc.fetchOpenAIImageURLBase64(ctx, b64BackfillAccount(true), "https://cdn.example.com/a.png")
	require.ErrorIs(t, err, context.Canceled)
	require.Empty(t, got)
	require.True(t, upstream.called)
}

type b64BackfillContextUpstream struct {
	called bool
}

func (u *b64BackfillContextUpstream) Do(req *http.Request, _ string, _ int64, _ int) (*http.Response, error) {
	u.called = true
	return nil, req.Context().Err()
}

func (u *b64BackfillContextUpstream) DoWithTLS(req *http.Request, proxyURL string, accountID int64, concurrency int, _ *tlsfingerprint.Profile) (*http.Response, error) {
	return u.Do(req, proxyURL, accountID, concurrency)
}

func TestFetchOpenAIImageURLBase64_RejectsDeclaredOversizeBeforeReadingBody(t *testing.T) {
	body := &b64BackfillUnreadableBody{}
	upstream := &httpUpstreamRecorder{resp: &http.Response{
		StatusCode:    http.StatusOK,
		ContentLength: openAIImageMaxDownloadBytes + 1,
	...
	require.Contains(t, err.Error(), "exceeds")
	require.Empty(t, got)
	require.False(t, body.read, "oversize Content-Length must be rejected before reading the response body")
	require.True(t, body.closed, "response body must still be closed")
	require.Len(t, upstream.requests, 1)
}

type b64BackfillUnreadableBody struct {
	read   bool
	closed bool
}

func (b *b64BackfillUnreadableBody) Read([]byte) (int, error) {
	b.read = true
	return 0, errors.New("body must not be read")
}

func (b *b64BackfillUnreadableBody) Close() error {
	b.closed = true
	return nil
}
