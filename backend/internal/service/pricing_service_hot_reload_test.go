...
// Personal regression retained from the earlier WP06 pass.
func TestPricingCustomLayersDeleteOnNextReloadButInvalidJSONKeepsSnapshot(t *testing.T) {
	dir := t.TempDir()
	catalog := filepath.Join(dir, "catalog.json")
	override := filepath.Join(dir, "override.json")
	dataDir := filepath.Join(dir, "data")
	require.NoError(t, os.MkdirAll(dataDir, 0755))
	body := []byte(`{"demo":{"input_cost_per_token":0.000001,"output_cost_per_token":0.000002}}`)
	require.NoError(t, os.WriteFile(catalog, body, 0644))
	require.NoError(t, os.WriteFile(filepath.Join(dataDir, "model_pricing.json"), body, 0644))
	require.NoError(t, os.WriteFile(override, []byte(`{"demo":{"input_cost_per_token":0.000009}}`), 0644))

	svc := &PricingService{cfg: &config.Config{Pricing: config.PricingConfig{DataDir: dataDir, FallbackFile: "", OverrideFile: override}}, pricingData: map[string]*LiteLLMModelPricing{}}
	require.NoError(t, svc.loadPricingData(catalog))
	require.InDelta(t, 0.000009, svc.pricingData["demo"].InputCostPerToken, 1e-15)

	require.NoError(t, os.Remove(override))
	svc.reloadIfCustomFilesChanged()
	require.InDelta(t, 0.000001, svc.pricingData["demo"].InputCostPerToken, 1e-15)

	require.NoError(t, os.WriteFile(override, []byte(`{invalid`), 0644))
	svc.reloadIfCustomFilesChanged()
	require.InDelta(t, 0.000001, svc.pricingData["demo"].InputCostPerToken, 1e-15)
}
