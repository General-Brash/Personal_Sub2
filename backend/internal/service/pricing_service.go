...
var (
	...
	openAIGPT55FallbackPricing = &LiteLLMModelPricing{
		InputCostPerToken: 5e-6, OutputCostPerToken: 30e-6, CacheCreationInputTokenCost: 5e-6, CacheReadInputTokenCost: 0.5e-6,
		InputCostPerTokenPriority: 12.5e-6, OutputCostPerTokenPriority: 75e-6, CacheCreationInputTokenCostPriority: 12.5e-6, CacheReadInputTokenCostPriority: 1.25e-6,
	...
	openAIGPT55ProFallbackPricing = &LiteLLMModelPricing{
		InputCostPerToken: 30e-6, OutputCostPerToken: 180e-6, CacheCreationInputTokenCost: 30e-6, CacheReadInputTokenCost: 30e-6,
		LongContextInputTokenThreshold: 272000, LongContextInputCostMultiplier: 2, LongContextOutputCostMultiplier: 1.5,
...
type LiteLLMModelPricing struct {
	...
	// LongContextInputTokenThresholdPresent is true when the source entry
	// explicitly carries a non-null threshold, including an explicit zero.
	// The zero value of the threshold alone cannot distinguish "disabled" from
	// an omitted/removed field, so preserve that source-level presence here.
	LongContextInputTokenThresholdPresent bool    `json:"-"`
...
func (s *PricingService) startUpdateScheduler() {
	...
	customEnabled := s.hasCustomPricingFiles()
...
func (s *PricingService) parsePricingData(body []byte) (map[string]*LiteLLMModelPricing, error) {
	...
	for modelName, rawEntry := range rawData {
		...
		pricing := &LiteLLMModelPricing{
			...
			LongContextInputTokenThresholdPresent: entry.LongContextInputTokenThreshold != nil,
		}
...
func (s *PricingService) useFallbackPricing() error {
	...
	pricingFile := s.getPricingFilePath()
	if err := os.WriteFile(pricingFile, data, 0644); err != nil { // #nosec G703 -- fixed filename under operator-owned Pricing.DataDir, not a request path.
...
func (s *PricingService) matchOpenAIModel(model string) *LiteLLMModelPricing {
	...
	// User-approved 0.2.1 fallback policy; configured catalog prices still win above.
