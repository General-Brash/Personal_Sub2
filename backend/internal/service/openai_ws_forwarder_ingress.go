...
func (s *OpenAIGatewayService) ProxyResponsesWebSocketFromClient(
	...
	sendAndRelay := func(turn int, lease *openAIWSConnLease, payload []byte, payloadBytes int, originalModel string, imageBillingModel string, imageSizeTier string, imageInputSize string) (*OpenAIForwardResult, error) {
		...
		// A native Responses WS turn may emit both error(server_error) and
		// response.failed(server_error). Account/model failure side effects are
		// request-level, so apply them once while preserving both client events.
		...
		for {
			...
			if eventType == "error" {
				s.applyOpenAIWSFailureSideEffectsOnce(ctx, account, mappedModel, lease.HandshakeHeaders(), eventType, upstreamMessage, &failureAccountSideEffectsApplied)
				...
				if s.persistOpenAIWSRateLimitSignal(ctx, account, lease.HandshakeHeaders(), upstreamMessage, errCodeRaw, errTypeRaw, errMsgRaw) {
			...
			if isTerminalEvent {
				...
				if terminalEvent == "response.failed" {
					s.applyOpenAIWSFailureSideEffectsOnce(ctx, account, mappedModel, lease.HandshakeHeaders(), terminalEvent, upstreamMessage, &failureAccountSideEffectsApplied)
				}
