...
func TestAccountTestPluginRoutingPreservesFallbackAndFailsClosed(t *testing.T) {
	manager := &PluginManager{}
	...
	service.SetPluginManager(manager)

	for _, useTLS := range []bool{false, true} {
		request, err := http.NewRequestWithContext(context.Background(), http.MethodPost, "https://example.com/v1/responses", nil)
		require.NoError(t, err)
		response, err := service.doOpenAIAccountTestUpstream(request, "", &Account{
			ID: 1, Platform: PlatformOpenAI, Type: AccountTypeAPIKey, Concurrency: 1,
		}, useTLS)
		require.NoError(t, err)
	...
	assert.Equal(t, 1, upstream.doWithTLSCalls)
