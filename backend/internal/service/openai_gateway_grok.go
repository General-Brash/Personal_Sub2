...
func (s *OpenAIGatewayService) forwardGrokResponses(
	...
	// Keep forwarding, scheduling cooldowns, and quota blocks on the same
	// account/runtime mapping source, including optional cross-client defaults.
	upstreamModel := resolveGrokCanonicalUpstreamModel(account, originalModel)
...
func (s *OpenAIGatewayService) handleGrokAccountUpstreamError(ctx context.Context, account *Account, statusCode int, headers http.Header, responseBody []byte) {
	...
	if strings.TrimSpace(decision.Model) != "" {
		decision.Model = resolveGrokCanonicalUpstreamModel(account, decision.Model)
	}
