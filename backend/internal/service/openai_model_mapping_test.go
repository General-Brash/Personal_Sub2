...
func TestCanonicalOpenAIAccountSchedulingModelMatchesForwardSemantics(t *testing.T) {
	originalOptions := xai.RuntimeModelMappingOptions()
	t.Cleanup(func() { xai.SetRuntimeModelMappingOptions(originalOptions) })
	...
	tests := []struct {
		...
		{
			name:    "Grok OAuth follows the enabled cross-client default",
...
func TestGrokBuildRuntimeModelCanonicalMatchesForwardAndScheduling(t *testing.T) {
	account := &Account{Platform: PlatformGrok, Type: AccountTypeOAuth}
	require.Equal(t, "grok-4.6", resolveGrokCanonicalUpstreamModel(account, "grok-4.6-build"))
	require.Equal(t, "grok-4.5", canonicalOpenAIAccountSchedulingModel(account, "grok-4.5-build"))
}
