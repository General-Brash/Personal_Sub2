...
func TestChannelMultipliersRejectNonFiniteValuesAcrossValidationAndBilling(t *testing.T) {
	for _, value := range []float64{0, -1, math.NaN(), math.Inf(1), math.Inf(-1)} {
		value := value
		t.Run(stringifyMultiplierTestValue(value), func(t *testing.T) {
			require.Error(t, checkPricesNotNegative(ChannelModelPricing{FastMultiplier: &value}))
			require.Error(t, checkPricesNotNegative(ChannelModelPricing{FlexMultiplier: &value}))
			require.Error(t, ValidateIntervals([]PricingInterval{{MinTokens: 1, InputMultiplier: &value}}, BillingModeToken))
			require.Error(t, validateChannelPricingMultipliers("model", &ChannelModelPricing{
				FastMultiplier: &value,
				Intervals:      []PricingInterval{{InputMultiplier: &value}},
			}))
			...
			require.Error(t, validateModelPricingMultipliers("model", "token", pricing))
			billing := &BillingService{fallbackPrices: map[string]*ModelPricing{"invalid": pricing}}
			require.Error(t, billing.PreflightTokenPricing(context.Background(), "invalid", nil, nil))
			...
			_, err := billing.calculateTokenCost(resolved, CostInput{
				Model: "invalid", Tokens: UsageTokens{InputTokens: 1}, RateMultiplier: 1,
...
func stringifyMultiplierTestValue(value float64) string {
	switch {
