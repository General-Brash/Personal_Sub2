...
func resolveGrokCanonicalUpstreamModel(account *Account, requestedModel string) string {
	...
	return xai.CanonicalGrokRuntimeModelID(model)
}
...
func canonicalOpenAIAccountSchedulingModel(account *Account, requestedModel string) string {
	...
	if account.IsGrok() {
		return resolveGrokCanonicalUpstreamModel(account, model)
	}
	if mapped := strings.TrimSpace(account.GetMappedModel(model)); mapped != "" {
		return mapped
	}
