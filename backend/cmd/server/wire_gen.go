...
func initializeApplication(buildInfo handler.BuildInfo) (*Application, error) {
	...
	temporaryCreditRepository := repository.NewTemporaryCreditRepository(db)
	bankService := service.ProvideBankService(db, temporaryCreditRepository, billingCache)
	...
	entitlementRepository := repository.NewEntitlementRepository(db, configConfig)
	entitlementService := service.NewEntitlementService(entitlementRepository)
	apiKeyService := service.ProvideAPIKeyService(apiKeyRepository, userRepository, groupRepository, userSubscriptionRepository, userGroupRateRepository, apiKeyCache, configConfig, billingCacheService, bankService, concurrencyService, entitlementService)
	...
	playerInvitationRepository := repository.NewPlayerInvitationRepository(client, db)
	adminPermissionRepository := repository.NewAdminPermissionRepository(db)
	adminPermissionService := service.ProvideAdminPermissionService(adminPermissionRepository)
	playerInvitationService := service.ProvidePlayerInvitationService(playerInvitationRepository, adminPermissionService, settingService, configConfig)
	authService := service.ProvideAuthService(client, userRepository, redeemCodeRepository, refreshTokenCache, configConfig, settingService, emailService, turnstileService, tencentCaptchaService, aliyunCaptchaService, emailQueueService, promoService, subscriptionService, affiliateService, serviceUserPlatformQuotaRepository, playerInvitationService)
	...
	usageService := service.ProvideUsageService(usageLogRepository, userRepository, client, apiKeyAuthCacheInvalidator, billingCacheService)
	...
	temporaryCreditService := service.ProvideTemporaryCreditService(temporaryCreditRepository, billingCacheService, settingService)
	temporaryCreditAuditRepository := repository.NewTemporaryCreditAuditRepository(db)
	adminTemporaryCreditService := service.NewAdminTemporaryCreditService(db, temporaryCreditService, temporaryCreditAuditRepository)
	temporaryCreditHandler := handler.ProvideAdminTemporaryCreditHandler(adminTemporaryCreditService)
	bankHandler := handler.ProvideAdminBankHandler(bankService)
	bankExchangeExpiryHandler := handler.ProvideAdminBankExchangeExpiryHandler(bankService)
	...
	dataCleanupService := service.ProvideDataCleanupService(db, settingService, usageCleanupService)
	dataCleanupHandler := admin.NewDataCleanupHandler(opsService, dataCleanupService, totpService)
	...
	adminHandlers := handler.ProvideAdminHandlers(dashboardHandler, adminUserHandler, temporaryCreditHandler, bankHandler, bankExchangeExpiryHandler, groupHandler, accountHandler, adminAnnouncementHandler, dataManagementHandler, backupHandler, oAuthHandler, openAIOAuthHandler, geminiOAuthHandler, antigravityOAuthHandler, grokOAuthHandler, cnProviderHandler, proxyHandler, adminRedeemHandler, promoHandler, settingHandler, opsHandler, dataCleanupHandler, systemHandler, adminSubscriptionHandler, adminUsageHandler, userAttributeHandler, errorPassthroughHandler, tlsFingerprintProfileHandler, pluginHandler, adminAPIKeyHandler, scheduledTestHandler, channelHandler, channelMonitorHandler, channelMonitorRequestTemplateHandler, contentModerationHandler, promptAdminHandler, paymentHandler, affiliateHandler, complianceHandler, auditLogHandler, oidcProviderHandler, upstreamBillingProbeService, ollamaCloudUsageService)
	...
	gatewayHandler := handler.ProvideGatewayHandler(gatewayService, openAIGatewayService, geminiMessagesCompatService, antigravityGatewayService, userService, concurrencyService, billingCacheService, usageService, apiKeyService, usageRecordWorkerPool, errorPassthroughService, contentModerationService, userMessageQueueService, configConfig, settingService, coordinator, entitlementService)
	openAIGatewayHandler := handler.ProvideOpenAIGatewayHandler(openAIGatewayService, pluginManager, concurrencyService, billingCacheService, apiKeyService, usageRecordWorkerPool, errorPassthroughService, contentModerationService, opsService, grokQuotaService, configConfig, coordinator, entitlementService)
	...
	mallService := service.ProvideMallService(db, temporaryCreditService, subscriptionService, apiKeyAuthCacheInvalidator)
	handlerPaymentHandler := handler.ProvidePaymentHandler(paymentService, paymentConfigService, mallService)
	...
	batchImagePublicService := service.ProvideBatchImagePublicService(batchImageRepository, accountRepository, groupRepository, userGroupRateRepository, batchImageQueue, batchImageModelPricingResolver, usageBillingRepository, apiKeyAuthCacheInvalidator, configConfig, entitlementService)
	...
	checkinService := service.NewCheckinServiceV2(db, settingService, temporaryCreditService)
	checkinHandler := handler.ProvideCheckinHandler(checkinService)
	checkinAdminHandler := handler.ProvideCheckinAdminHandler(settingService)
	handlerBankHandler := handler.ProvideBankHandler(bankService)
	invitationHandler := handler.NewInvitationHandler(authService)
	featureManagementHandler := handler.NewFeatureManagementHandler(adminPermissionService, entitlementService, settingService)
	dynamicRateHandler := handler.ProvideDynamicRateHandler(gatewayService, apiKeyService)
	...
	opsCleanupService := service.ProvideOpsCleanupService(opsRepository, db, redisClient, configConfig, channelMonitorService, settingRepository, opsService, usageCleanupService)
	...
	affiliateRebateWorker := service.ProvideAffiliateRebateWorker(client, affiliateService, settingService)
