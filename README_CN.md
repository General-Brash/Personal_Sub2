...
Personal_Sub2 是基于 [Sub2API](https://github.com/Wei-Shaw/sub2api) 维护的个人分支，在保留上游网关基础能力的同时，为本部署独立维护扩展功能。本项目不是上游 Sub2API 官方项目，也不代表或冒充上游维护者。

相较上游 Sub2API，本分支目前新增：

- **临时额度** - 在永久余额之外，记录具有有效期的临时额度发放、消费和可用额度。
- **每日签到** - 按可配置规则向用户发放临时额度奖励，并提供签到状态和记录。
- **银行** - 支持预支临时额度、将永久额度兑换为临时额度，并提供可配置的限额、结算规则和流水记录。
- **安全审计二次审核** - 改进了英文等 ASCII 关键词的边界匹配；命中后可交给独立 `intent-classifier` 服务二次判定。集成支持 `off`、`shadow`、`enforce` 模式，以及版本化 ONNX 模型包、健康与就绪状态、校验、激活和回滚。

仓库不包含正式模型权重。启用 `shadow` 或 `enforce` 的模型判定前，运维人员必须按照 [`MODEL_PACKAGE.md`](services/intent-classifier/MODEL_PACKAGE.md) 提供并激活模型包。
