...
async function openConfiguration(plugin: PluginInstallation): Promise<void> {
  ...
  try {
    ...
    if (
      requestSequence !== configurationRequestSequence ||
      configPlugin.value?.id !== plugin.id
    ) {
      return;
    }
...
function closeConfiguration(): void {
  configurationRequestSequence += 1;
  clearPendingBridgeRequests();
  pluginFrameLoaded.value = false;
...
function registerBridgeRequest(message: PluginBridgeMessage): void {
  ...
  pending.timeout = window.setTimeout(() => {
    if (pendingBridgeRequests.get(requestID) === pending) {
...
function postBridgeResult(
  ...
  if (!requestID || !pending || pending.message !== request) return;
  window.clearTimeout(pending.timeout);
  pendingBridgeRequests.delete(requestID);
