...
interface PluginDocumentSession {
...
interface PendingBridgeRequest extends PluginDocumentSession {
  requestID: string;
...
let pluginDocumentGeneration = 0;
let frameLoadHandled = false;
let currentPluginDocument: PluginDocumentSession | null = null;
...
function isCurrentConfigurationRequest(requestSequence: number, pluginID: number): boolean {
  return (
    configurationRequestSequence === requestSequence &&
    configPlugin.value?.id === pluginID
  );
}

function invalidatePluginDocument(): void {
  ++pluginDocumentGeneration;
  frameLoadHandled = false;
  currentPluginDocument = null;
  clearPendingBridgeRequests();
}

function isCurrentPluginDocument(
  documentSession: PluginDocumentSession,
  session: PluginUISession,
  ...
  return (
    ...
    documentSession.pluginID === plugin.id &&
    documentSession.sourceWindow === sourceWindow &&
    documentSession.bridgeToken === session.bridge_token &&
    documentSession.configurationSequence === configurationRequestSequence
  );
}

function createPluginDocumentSession(
  session: PluginUISession,
  plugin: PluginInstallation,
  sourceWindow: Window,
  ready: boolean,
): PluginDocumentSession {
  return {
    documentGeneration: ++pluginDocumentGeneration,
    sourceWindow,
    bridgeToken: session.bridge_token,
    session,
    configurationSequence: configurationRequestSequence,
    pluginID: plugin.id,
    ready,
  };
}

function ensurePluginDocumentSession(
  session: PluginUISession,
  plugin: PluginInstallation,
  sourceWindow: Window,
): PluginDocumentSession {
  const current = currentPluginDocument;
  if (current && isCurrentPluginDocument(current, session, plugin, sourceWindow)) {
    return current;
  }

  clearPendingBridgeRequests();
  frameLoadHandled = false;
  const next = createPluginDocumentSession(session, plugin, sourceWindow, false);
  currentPluginDocument = next;
  return next;
}

function markPluginDocumentReady(
  session: PluginUISession,
  plugin: PluginInstallation,
  sourceWindow: Window,
): PluginDocumentSession {
  ...
  if (current && isCurrentPluginDocument(current, session, plugin, sourceWindow)) {
    if (current.ready) {
      // A second ready handshake marks a new iframe document, even when the
      // browser has not emitted that document's load event yet.
      clearPendingBridgeRequests();
      frameLoadHandled = false;
      const next = createPluginDocumentSession(session, plugin, sourceWindow, true);
      currentPluginDocument = next;
      return next;
    }
  ...
  const next = ensurePluginDocumentSession(session, plugin, sourceWindow);
  next.ready = true;
  return next;
}

async function openConfiguration(plugin: PluginInstallation): Promise<void> {
  const requestSequence = ++configurationRequestSequence;
  const pluginID = plugin.id;
  invalidatePluginDocument();
  ...
  try {
    const session = await adminAPI.plugins.createUISession(pluginID);
    if (!isCurrentConfigurationRequest(requestSequence, pluginID)) return;
...
function handlePluginFrameLoad(): void {
  // The plugin can send ready/config.load before the parent receives iframe
  // load. Preserve that early session for the first document, but treat every
  // later load as a navigation boundary for the previous document.
  const session = uiSession.value;
  const plugin = configPlugin.value;
  const sourceWindow = pluginFrame.value?.contentWindow;
  if (!session || !plugin || !sourceWindow) return;

  if (frameLoadHandled) {
    clearPendingBridgeRequests();
    currentPluginDocument = createPluginDocumentSession(
      session,
      plugin,
      sourceWindow,
      false,
    ...
    ensurePluginDocumentSession(session, plugin, sourceWindow);
  }
  frameLoadHandled = true;
  uiLoading.value = false;
}

function isCurrentBridgeRequest(pending: PendingBridgeRequest): boolean {
  const documentSession = currentPluginDocument;
  return (
    !!documentSession &&
    pendingBridgeRequests.get(pending.requestID) === pending &&
    pending.documentSession === documentSession &&
    pending.documentGeneration === documentSession.documentGeneration &&
    pending.sourceWindow === documentSession.sourceWindow &&
    pending.sourceWindow === pluginFrame.value?.contentWindow &&
    pending.bridgeToken === documentSession.bridgeToken &&
    pending.session === documentSession.session &&
    pending.configurationSequence === documentSession.configurationSequence &&
    pending.pluginID === documentSession.pluginID &&
    pending.session === uiSession.value &&
    pending.pluginID === configPlugin.value?.id
  );
}
...
function registerBridgeRequest(
  ...
  pending.timeout = window.setTimeout(() => {
    if (pendingBridgeRequests.get(pending.requestID) === pending) {
      pendingBridgeRequests.delete(pending.requestID);
    }
  }, 30_000);
  pendingBridgeRequests.set(pending.requestID, pending);
  return pending;
}
...
async function handleBridgeMessage(event: MessageEvent): Promise<void> {
  ...
  if (
    ...
    message.bridge_token !== session.bridge_token
  )
    return;

  let documentSession: PluginDocumentSession;
  if (message.type === "sub2api.plugin.ready") {
    markPluginDocumentReady(session, plugin, sourceWindow);
    uiLoading.value = false;
    return;
  }
  documentSession = ensurePluginDocumentSession(session, plugin, sourceWindow);
  ...
  let pending: PendingBridgeRequest | null = null;
  if (expectsResponse) {
    if (!requestID || pendingBridgeRequests.has(requestID)) return;
    pending = registerBridgeRequest(documentSession, requestID);
  }
  ...
  try {
    switch (message.type) {
      case "config.load": {
        if (!pending) return;
        const config = await adminAPI.plugins.getConfig(pending.pluginID);
        if (!isCurrentBridgeRequest(pending)) return;
      ...
      case "config.save": {
        ...
        if (!isCurrentBridgeRequest(pending)) return;
        if (postBridgeResult(message, { ok: true, config }, pending)) {
      ...
      case "config.test": {
        if (!pending) return;
        const result = await pluginStepUp.run(() =>
          adminAPI.plugins.test(pending.pluginID),
        );
        if (!isCurrentBridgeRequest(pending)) return;
        if (!postBridgeResult(message, { ok: result.success, result }, pending)) return;
        if (result.success)
    ...
    if (!pending || !isCurrentBridgeRequest(pending)) return;
