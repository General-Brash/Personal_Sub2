...
describe('AppSidebar page visibility navigation', () => {
  ...
  it('binds administrator finance, bank, audit, and ops entries to page flags', () => {
    ...
    expect(componentSource).toContain("const flagPluginManagement = makeSidebarFlag(FeatureFlags.pluginManagement)")
    ...
    expect(componentSource).toContain("path: '/admin/plugins', label: t('nav.plugins'), icon: PluginIcon, featureFlag: flagPluginManagement")
  })
})
