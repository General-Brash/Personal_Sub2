      ...
      <div class="flex min-w-0 items-center gap-1 sm:gap-3">
        ...
        >
          ...
          <span class="text-sm font-semibold text-indigo-600 dark:text-indigo-400">
            {{ balancePermanentText }} {{ formatHeaderMoney(availableBalance) }}
          </span>
          <span class="text-sm font-semibold text-emerald-600 dark:text-emerald-400">
            {{ balanceTemporaryText }} {{ formatHeaderMoney(temporaryCreditAvailable) }}
          </span>
          <span
          ...
          >
            <div class="flex items-center justify-between">
              <span class="text-gray-500 dark:text-dark-400">{{ balancePermanentText }}</span>
              <span class="font-medium text-indigo-600 dark:text-indigo-400">{{ formatHeaderMoney(availableBalance) }}</span>
            </div>
            <div class="mt-2 flex items-center justify-between">
              <span class="text-gray-500 dark:text-dark-400">{{ balanceTemporaryText }}</span>
              <span class="font-medium text-emerald-600 dark:text-emerald-400">{{ formatHeaderMoney(temporaryCreditAvailable) }}</span>
            </div>
            ...
            <div v-if="dropdownOpen" class="dropdown right-0 mt-2 w-56">
              ...
              <div class="py-1">
                ...
                <a
                  ...
                  href="https://github.com/General-Brash/Personal_Sub2"
                  target="_blank"
                  rel="noopener noreferrer"
                  @click="closeDropdown"
                  class="dropdown-item"
...
// 模型广场 V2 已彻底接管入口:仅看 model_plaza_v2_enabled(opt-out,缺省视为开启)。
const modelPlazaEnabled = computed(() => appStore.cachedPublicSettings?.model_plaza_v2_enabled !== false)
const avatarUrl = computed(() => user.value?.avatar_url?.trim() || '')
const availableBalance = computed(() => Number(user.value?.balance || 0))
const temporaryCreditAvailable = computed(() => Number(user.value?.temporary_credit_available || 0))
const frozenBalance = computed(() => Number(user.value?.frozen_balance || 0))
const totalBalance = computed(() => availableBalance.value + frozenBalance.value)
const balancePermanentText = computed(() => t('common.permanentBalance') === 'common.permanentBalance' ? '永久额度' : t('common.permanentBalance'))
const balanceTemporaryText = computed(() => t('common.temporaryCreditAvailable') === 'common.temporaryCreditAvailable' ? '临时额度' : t('common.temporaryCreditAvailable'))
