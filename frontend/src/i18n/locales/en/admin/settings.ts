export default {
    settings: {
      ...
      security: {
        ...
        oidcConsentPromptMode: 'OIDC Consent Page Policy',
        oidcConsentPromptModeHint:
          'Controls whether the consent page is shown when third-party apps sign in through this site. This setting overrides each OIDC client\'s "Trusted consent" policy; silent prompt=none checks are unaffected.',
        oidcConsentPromptModeAlways: 'Ask every time',
        oidcConsentPromptModeRemember: 'Remember after first approval'
      },
      panelRateLimit: {
      ...
      quickJump: {
        title: 'Header Quick Jump',
        description: 'Add a quick-jump entry to the site header. Users open it and jump to any of the external links configured here (opened in a new tab).',
        enabled: 'Show Header Entry',
        enabledHint: 'When off, the header button is hidden but the configured links are kept.',
        itemLabel: 'Jump Item #{n}',
        name: 'Button Label',
        namePlaceholder: 'e.g. Community Forum',
        url: 'Target URL',
      ...
      entitlementPolicy: {
        title: 'Standard / Premium Tier Policy',
        description: 'Roles, spending tiers, and business groups are independent. Saving a policy, toggling it, and assigning tiers to users are three separate actions; this does not migrate users automatically or assign tiers by group name.',
        load: 'Load tier policy',
        reload: 'Reload tier policy',
        loading: 'Loading…',
        emptyTiers: 'No tier policy to read; nothing is auto-created and no seed is re-run.',
        enabled: 'Enabled',
        disabled: 'Not enabled',
        noGroups: 'none',
        tierSummary: '{tier}: {status} · version {version} · linked group IDs: {groups}',
        displayName: 'Tier name',
        groupIdLabel: 'Linked group',
        groupNameLabel: 'Group name',
        groupPlatformLabel: 'Group platform',
        groupDeleted: 'Group deleted or unknown',
        rateLabel: 'Default multiplier (blank inherits)',
        sourceTierCustom: 'Effective source: tier custom {rate}x',
        sourceGroupDefault: 'Effective source: blank · inherits group default',
        sourceGroupDefaultWithRate: 'Effective source: blank · inherits group default {rate}x',
        remove: 'Remove',
        addGroup: 'Add tier group',
        reasonLabel: 'Change reason (audit required)',
        contentNote: 'Manual per-user multipliers take priority over tier defaults; manual groups and active subscriptions are preserved. Saving content does not change the current enabled state or auto-assign tiers.',
        reasonRequired: 'Enter an audit reason before saving or toggling.',
        saveContent: 'Save policy content',
        saving: 'Saving…',
        disablePremium: 'Disable premium only',
        enablePremium: 'Enable premium only',
        editsPendingNote: 'Save the policy content first, then enable or disable separately. After enabling, still preview and apply tiers from the user list.',
        pendingUnknown: 'The original request result for {tier} is unknown; reuse the original ID to reconcile/retry, do not write with a new ID.',
        requestId: 'Request ID: {id}',
        retryOriginal: 'Reconcile/retry with the original request',
        savedNotice: '{tier} policy saved: {status}, version {version}, linked group IDs {groups}. No users were migrated automatically; next, preview and apply tiers from the user list.',
        savedRefreshFailed: ' Saved successfully, but refresh failed; please reread, do not save again.',
        cap: {
          loading: 'Reading the policy and verifying write capability.',
          unknown: 'Write capability is unknown; read the policy first. Missing capability is not treated as granted write access.',
          noPermission: 'You do not have users.entitlement.manage write permission.',
          writesDisabled: 'The backend has not enabled this kind of write.',
          requiresReload: 'State changed; reread and confirm. Nothing is overwritten automatically.',
          verified: 'users.entitlement.manage write capability verified.',
        },
        err: {
          groupIdPositive: 'Group ID must be a positive integer.',
          groupIdDuplicate: 'The same group ID cannot repeat.',
          rateRange: 'Default multiplier must be between 0 and 1000.',
          readForbidden: 'You do not have users.entitlement.manage catalog read permission.',
          readFailed: 'Failed to read the tier policy, please retry.',
          unknownRetry: 'The original request result is unknown; reconcile/retry with the same request ID.',
          casConflict: 'Policy version or state changed; nothing was written. Reread and confirm; nothing was overwritten automatically.',
          writeForbidden: 'You do not have write permission; nothing was written.',
          writeRejected: 'The request was rejected; check the policy content and audit reason.',
        },
      },
      userEntitlement: {
        title: 'Entitlement',
        targetsSummary: '{count} target(s); the current entitlement of the first user is shown below. Roles, spending tiers, and business groups are independent.',
        verifying: 'Verifying',
        writable: 'Writable',
        readOnly: 'Read-only',
        catalogErrorSuffix: '; the basic entitlement that was read successfully is still viewable.',
        pendingUnknown: 'The original request result is unknown; do not reassign with a new request. Targets: {targets}; tier: {tier}.',
        requestId: 'Request ID: {id}',
        storageUnavailable: 'Cannot store reconciliation info; keep this window open until reconciliation completes.',
        checking: 'Reconciling…',
        retryOriginal: 'Reconcile/retry with the original request',
        reload: 'Reread entitlement and policy',
        loadingBasic: 'Loading basic entitlement…',
        currentTier: 'Current effective spending tier',
        version: 'Entitlement version',
        premiumPolicy: 'Premium policy',
        previewTier: 'Preview tier {tier}',
        applyNote: 'Saving or enabling a policy does not migrate users automatically. Assignment preserves manual multipliers, manual groups, standalone entitlement grants, and active subscriptions; a standalone grant may keep premium.',
        reasonLabel: 'Original reason (audit required)',
        reasonPlaceholder: 'e.g. campaign grant / user appeal adjustment',
        previewing: 'Previewing, cannot apply yet…',
        previewSummary: 'Will update base assignment for {affected} user(s), no change needed for {already} user(s).',
        previewGroups: 'Newly available groups {granted}; revoked groups {revoked}. Manual groups and active subscriptions are preserved.',
        previewMeta: 'Policy version {version}; preview valid until {expires}.',
        previewDisabled: 'This tier policy is not enabled: this preview is read-only and cannot be applied.',
        applying: 'Applying…',
        confirmApply: 'Confirm apply',
        sourcesTitle: 'Source explanation',
        defaultStandard: 'Default standard',
        manualGroups: 'Manual groups: ',
        subscriptionGroups: 'Active subscription groups: ',
        none: 'none',
        reading: 'Reading',
        policyState: '{status} · v{version}',
        enabled: 'Enabled',
        disabled: 'Not enabled',
        unknownPolicy: 'Unknown (policy not read)',
        replaySuffix: ' (original result replayed)',
        savedNotice: 'Targets {targets} assignment saved: {changed} changed, {unchanged} unchanged; version {version}{replay}.',
        savedRefreshFailed: ' Saved successfully, but refresh failed; please reread, do not write again.',
        ro: {
          verifying: 'Verifying the current entitlement and write capability.',
          unknown: 'Write capability is unknown; missing capability info is not treated as granted permission.',
          noPermission: 'The current administrator does not have users.entitlement.manage write capability.',
          writesDisabled: 'The backend has not enabled this kind of write.',
        },
        err: {
          basicForbidden: 'No users.read permission; cannot read basic entitlement.',
          basicFailed: 'Failed to read basic entitlement, please retry.',
          catalogForbidden: 'No tier policy catalog read permission',
          catalogFailed: 'Failed to read the tier policy catalog',
          previewToken: 'No valid server preview token received; preview again. The version guard is not bypassed.',
          previewExpired: 'Preview expired, please preview again.',
          previewFailed: 'Preview failed, please preview again.',
          unknownRetry: 'The original request result is not confirmed yet; reconcile/retry with the same request ID.',
          casConflict: 'Preview or state changed; nothing was written. Reread and preview again.',
          writeForbidden: 'You currently have no write permission; nothing was written.',
          writeRejected: 'The request was rejected; nothing was written. Check and preview again.',
        },
      },
