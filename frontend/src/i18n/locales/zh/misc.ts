export default {
  ...
  payment: {
    ...
    purchaseLimit: {
      ...
      calendarWeek: '自然周',
      calendarMonth: '自然月',
      rollingDay: '滚动 {count} 天',
      rollingWeek: '滚动 {count} 周',
      rollingMonth: '滚动 {count} 月',
      total: '总计',
      unlimited: '不限次数',
      exhausted: '已达限购上限',
      detail: '{scope}已购 {used}/{limit}，剩余 {remaining} 次',
      windowHint: '限购周期：{scope}',
    },
    purchaseConfirm: {
      title: '确认购买',
      paymentMethod: '支付额度',
      expectedSpend: '预计消费',
      expectedReceive: '预计获得',
      remainingTitle: '当前剩余购买次数',
      subscriptionReceive: '{validity}订阅权益，倍率 ×{rate}',
      confirm: '确认购买',
    },
    status: {
      pending: '待支付',
    ...
    errors: {
      ...
      DAILY_PURCHASE_LIMIT_EXCEEDED: '该商品今日购买次数已达上限',
      TOTAL_PURCHASE_LIMIT_EXCEEDED: '该商品累计购买次数已达上限',
    ...
    admin: {
      ...
      price: '价格',
      benefitType: '权益类型',
      benefitSub2: 'Sub2 订阅权益',
      benefitDailyTemporaryCredit: '每日临时额度',
      paymentCreditType: '支付额度类型',
      dailyTemporaryCreditAmount: '每日发放临时额度',
      dailyTemporaryCreditAmountRequired: '每日发放临时额度必须是大于 0 的数值。',
      dailyTemporaryCreditHint: '按北京时间每日发放；有效期内续订只顺延，不叠加每日额度。',
      currency: '币种标注',
      ...
      purchaseLimit: '限购策略',
      periodicPurchaseLimit: '每位用户周期限购',
      totalPurchaseLimit: '每位用户总限购',
      purchaseLimitPlaceholder: '0（不限）',
      periodicPurchaseLimitHint: '0 或留空表示不限；周期由下方单位和刷新模式决定。',
      totalPurchaseLimitHint: '用户历史累计购买上限；0 或留空表示不限。',
      purchaseLimitUnit: '限购单位',
      purchaseLimitUnitDay: '日',
      purchaseLimitUnitWeek: '周',
      purchaseLimitUnitMonth: '月',
      purchaseLimitMode: '刷新模式',
      purchaseLimitModeCalendar: '自然周期',
      purchaseLimitModeRolling: '滚动窗口',
      purchaseLimitWindowSize: '滚动长度',
      purchaseLimitWindowSizeHint: '与限购单位组合，例如 3 天、2 周、4 月。',
      purchaseLimitCalendarHint: '自然周期固定为 1 个所选单位。',
      purchaseLimitSummary: '{scope}，每人限购 {limit} 次',
      purchaseLimitInvalid: '限购次数必须是大于或等于 0 的整数。',
      purchaseLimitWindowSizeInvalid: '滚动长度必须是大于 0 的整数。',
