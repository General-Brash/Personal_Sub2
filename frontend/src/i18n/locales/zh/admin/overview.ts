export default {
    ...
    groups: {
      ...
      form: {
        ...
        maxReasoningEffortHint: '限制客户端主动请求的 Anthropic/OpenAI 推理强度；Composite 分组按实际目标平台生效。缺省 effort 保持缺省。上限优先级高于推理强度映射。',
        maxReasoningEffortOverLimit: '超限访问控制',
        maxReasoningEffortOverLimitDowngrade: '超过上限时自动降档',
        maxReasoningEffortOverLimitDeny: '拒绝访问',
        maxReasoningEffortOverLimitHint: '设置上限后生效。自动降档会将超过上限的请求改写为上限值后转发；拒绝访问则直接返回错误。',        reasoningEffortMappings: '推理强度映射',
        ...
        reasoningEffortModelPlaceholder: '留空则全部 / gpt / gpt-5.4',        addReasoningEffortMapping: '添加映射',
        ...
        duplicateScope: '类型和模型组合不能重复',        duplicateFrom: '请求值 A 不能重复',
        exclusiveLabel: '专属分组',
      ...
      codexModelsManifest: {
        ...
        hint: '启用后，该分组的 Codex 客户端 /models 请求只从固定账号获取并按 slug 合并，绕过调度器。限流或过载窗口中的固定账号仍会使用。',
        enable: '使用特定账号获取 manifest',
        enabledHint: '账号来源限定为当前分组绑定的 OpenAI 账号，最多选择 10 个。',
        disabledHint: '未启用：manifest 请求仍由调度器选取账号。',
        accounts: '固定账号',
        searchPlaceholder: '搜索账号（当前分组的 OpenAI 账号）',
        searchEmpty: '未找到匹配账号',
        fallback: '固定账号全部不可用时回退到调度器',
        fallbackHint: '关闭时返回 503 / 上游错误；开启时回退到现有调度器路径。',
        selectAtLeastOne: '启用固定账号后至少选择一个账号'
      ...
      openaiFast: {
        ...
        free: '免费 Fast',
        freeHint: 'Fast 请求仍使用 priority 档位，但向客户按 Standard 等价价格计费。'
