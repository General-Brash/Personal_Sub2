    ...
    settings: {
      ...
      features: {
        pageVisibility: {
          ...
          mall: {
            label: '商城',
            hint: '控制普通用户侧边栏中的商城页面及地址直接访问；银行页面不受影响。',
          },
          userSubscriptions: {
            label: '我的订阅',
      ...
      security: {
        ...
        oidcConsentPromptMode: 'OIDC 授权页策略',
        oidcConsentPromptModeHint:
          '控制第三方应用通过本站登录时是否展示授权页。此设置优先于各 OIDC 客户端的「Trusted consent 策略」；prompt=none 的静默检查不受影响。',
        oidcConsentPromptModeAlways: '每次都要用户确认',
        oidcConsentPromptModeRemember: '首次确认后记住'
      },
      panelRateLimit: {
        title: '面板接口限流',
      ...
      quickJump: {
        title: '顶部快捷跳转',
        description: '在站点顶栏添加一个快捷跳转入口，用户点开后可一键跳转到这里配置的外部链接（新标签页打开）。',
        enabled: '展示顶部入口',
        enabledHint: '关闭后顶栏不再显示快捷跳转按钮，已配置的链接会保留。',
        itemLabel: '跳转项 #{n}',
        name: '按钮名称',
        namePlaceholder: '如：社区论坛',
        url: '跳转链接',
      ...
      entitlementPolicy: {
        title: '普通／优质用户等级政策',
        description: '管理角色、消费等级、业务分组互相独立。保存政策、启停政策和对用户赋级是三个独立操作；这里不会自动迁移用户或按组名赋级。',
        load: '加载等级政策',
        reload: '重新读取等级政策',
        loading: '读取中…',
        emptyTiers: '没有可读取的等级政策；不会自动创建政策或重跑 seed。',
        enabled: '已启用',
        disabled: '未启用',
        noGroups: '无',
        tierSummary: '{tier}：{status} · 版本 {version} · 关联组 ID：{groups}',
        displayName: '等级名称',
        groupIdLabel: '关联分组',
        groupNameLabel: '分组名称',
        groupPlatformLabel: '分组平台',
        groupDeleted: '分组已删除或未知',
        rateLabel: '默认倍率（留空继承）',
        sourceTierCustom: '生效来源：tier 自定义 {rate}x',
        sourceGroupDefault: '生效来源：留空 · 继承分组默认倍率',
        sourceGroupDefaultWithRate: '生效来源：留空 · 继承分组默认 {rate}x',
        remove: '移除',
        addGroup: '增加等级分组',
        reasonLabel: '变更原因（审计必填）',
        contentNote: '个人手工倍率优先于等级默认；手工分组和有效订阅保留。保存内容不会改变当前启用状态，也不会给用户自动赋级。',
        reasonRequired: '填写审计原因后方可保存或启停。',
        saveContent: '保存政策内容',
        saving: '保存中…',
        disablePremium: '单独停用 premium',
        enablePremium: '单独启用 premium',
        editsPendingNote: '请先保存政策内容，再单独启用或停用。启用后仍需到用户列表预览并应用赋级。',
        pendingUnknown: '{tier} 的原请求结果未知；请使用原编号核对／重试，不要另换编号写入。',
        requestId: '请求编号：{id}',
        retryOriginal: '使用原请求核对／重试',
        savedNotice: '{tier} 政策已保存：{status}，版本 {version}，关联组 ID {groups}。未自动迁移用户；下一步可到用户列表预览并应用赋级。',
        savedRefreshFailed: ' 保存成功，但刷新失败；请重新读取，不要重复保存。',
        cap: {
          loading: '正在读取政策并核验写入能力。',
          unknown: '写入能力未知，请先读取政策；不会把能力缺失当作已获得写入权限。',
          noPermission: '当前没有 users.entitlement.manage 写入权限。',
          writesDisabled: '后端未开启该类写入。',
          requiresReload: '状态已变化，需重新读取后确认；不会自动覆盖。',
          verified: '已核验 users.entitlement.manage 写入能力。',
        },
        err: {
          groupIdPositive: '分组 ID 必须为正整数。',
          groupIdDuplicate: '同一分组 ID 不能重复。',
          rateRange: '默认倍率必须在 0 到 1000 之间。',
          readForbidden: '没有 users.entitlement.manage 目录读取权限。',
          readFailed: '读取等级政策失败，请重试读取。',
          unknownRetry: '原请求结果未知，请使用同一请求编号核对／重试。',
          casConflict: '政策版本或状态已变化，本次未写入；请重新读取后确认，未自动覆盖。',
          writeForbidden: '当前没有写入权限，本次未写入。',
          writeRejected: '请求被拒绝，请核对政策内容和审计原因。',
        },
      },
      userEntitlement: {
        title: '消费权益',
        targetsSummary: '目标 {count} 人；下方显示首位用户当前权益。管理角色、消费等级和业务分组互相独立。',
        verifying: '核验中',
        writable: '可写',
        readOnly: '只读',
        catalogErrorSuffix: '；已成功读取的基础权益仍可查看。',
        pendingUnknown: '原请求结果未知，不能另换请求重新赋级。目标：{targets}；等级：{tier}。',
        requestId: '请求编号：{id}',
        storageUnavailable: '无法保存核对信息，请保留本窗口直到核对完成。',
        checking: '核对中…',
        retryOriginal: '使用原请求核对／重试',
        reload: '重新读取权益与政策',
        loadingBasic: '加载基础权益中…',
        currentTier: '当前有效消费等级',
        version: '权益版本',
        premiumPolicy: 'Premium 政策',
        previewTier: '预览赋级 {tier}',
        applyNote: '保存或启用政策不会自动迁移用户。赋级保留手工倍率、手工分组、独立权益 grant 与有效订阅；独立 grant 可能继续维持 premium。',
        reasonLabel: '原始原因（审计必填）',
        reasonPlaceholder: '例如：活动授予 / 用户申诉调整',
        previewing: '预览中，暂不可应用…',
        previewSummary: '将更新基础赋级 {affected} 人，无需更改基础赋级 {already} 人。',
        previewGroups: '新增可用组 {granted}；撤销可用组 {revoked}。手工分组和有效订阅保留。',
        previewMeta: '政策版本 {version}；预览有效至 {expires}。',
        previewDisabled: '该等级政策未启用：本次预览只读，不可应用。',
        applying: '应用中…',
        confirmApply: '确认应用',
        sourcesTitle: '来源解释',
        defaultStandard: '默认 standard',
        manualGroups: '手工分组：',
        subscriptionGroups: '有效订阅组：',
        none: '无',
        reading: '读取中',
        policyState: '{status} · v{version}',
        enabled: '已启用',
        disabled: '未启用',
        unknownPolicy: '未知（未读取政策）',
        replaySuffix: '（原请求结果重放）',
        savedNotice: '目标 {targets} 赋级已保存：变更 {changed} 人，无需更改 {unchanged} 人；版本 {version}{replay}。',
        savedRefreshFailed: ' 保存成功，但刷新失败；请重新读取，不要重复写入。',
        ro: {
          verifying: '正在核验当前权益与写入能力。',
          unknown: '写入能力未知；不会将缺失能力信息视为已获得权限。',
          noPermission: '当前管理员没有 users.entitlement.manage 写入能力。',
          writesDisabled: '后端未开启该类写入。',
        },
        err: {
          basicForbidden: '没有 users.read 权限，无法读取基础权益。',
          basicFailed: '基础权益读取失败，请重试读取。',
          catalogForbidden: '没有等级政策目录读取权限',
          catalogFailed: '等级政策目录读取失败',
          previewToken: '未收到有效的服务端预览凭证，请重新预览；不会绕过版本守卫写入。',
          previewExpired: '预览已过期，请重新预览。',
          previewFailed: '预览失败，请重新预览。',
          unknownRetry: '尚未取得原请求的确定结果，请使用同一请求编号核对／重试。',
          casConflict: '预览或状态已变化，本次未写入；请重新读取并预览。',
          writeForbidden: '当前无写入权限，本次未写入。',
          writeRejected: '请求被拒绝，本次未写入；请核对后重新预览。',
        },
      },
      saveSettings: '保存设置',
      saving: '保存中...',
