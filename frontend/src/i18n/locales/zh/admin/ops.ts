export default {
    ops: {
      ...
      settings: {
        ...
        retentionDaysHint: '建议保留 7-90 天，过长会占用存储空间；填 0 表示禁用该项自动清理',
        aggregation: '预聚合任务',
        ...
        validation: {
          title: '请先修正以下问题',
          retentionDaysRange: '保留天数必须在 0-365 天之间（0 = 禁用自动清理）',
      ...
      cleanup: {
        title: '数据清理中心',
        entryHint: '配置自动保留策略，预览并执行受审计保护的手动清理',
        loadFailed: '加载清理配置失败',
        retry: '重试',
        saveSuccess: '自动清理策略已保存',
        saveFailed: '保存自动清理策略失败',
        automaticTitle: '自动清理',
        automaticHint: '每类数据独立启用；保留天数为 0 时禁用，绝不会自动清空整表。审计日志继续使用原有独立保留任务。',
        schedule: '执行计划（Cron）',
        cronHint: '使用标准五段 Cron，例如 0 2 * * *。',
        cronInvalid: '请输入有效的五段 Cron 表达式。',
        auditRetention: '操作审计日志保留天数（0 = 永久保留）',
        manualTitle: '手动清理',
        category: '数据类别',
        mode: '范围',
        rangeMode: '按日期范围',
        ...
        endDate: '结束日期（不含当日）',
        preview: '预览',
        previewRows: '可删除 {count} 条',
        blockedRows: '{count} 条因关联关系受保护',
        rangeRequired: '请选择开始和结束日期',
        previewFailed: '清理预览失败',
        sensitiveHint: '此类别包含敏感财务或安全记录，执行时必须输入确认词和现场 TOTP。',
        confirmation: '输入确认词：{value}',
        totp: '六位 TOTP 验证码',
        execute: '执行清理',
        executeSuccess: '清理请求已提交',
        executePending: '清理任务已进入队列，可在审计列表查看进度',
        executeFailed: '清理执行失败',
        auditTitle: '清理审计',
        auditLoadFailed: '加载清理审计失败',
        auditPending: '存在正在排队或执行的清理任务，列表将自动刷新。',
        auditTime: '时间',
        operator: '操作人',
        status: '状态',
        deletedRows: '实际删除',
        ...
        targets: {
          opsErrorLogs: '运维错误日志',
          opsAlertEvents: '运维告警事件',
          opsSystemLogs: '运维系统日志',
          opsCleanupAudits: '系统日志清理审计',
          opsSystemMetrics: '分钟系统指标',
          opsMetricsHourly: '小时聚合指标',
          opsMetricsDaily: '日聚合指标',
          usageLogs: '全站使用流水',
          auditLogs: '操作审计日志',
          mallPurchases: '商城交易记录',
          paymentOrders: '支付订单',
          bankLedger: '银行交易日志'
        }
      },
