...
# Sub2API 安装脚本
# Usage: curl -sSL https://raw.githubusercontent.com/General-Brash/Personal_Sub2/main/deploy/install.sh | bash
#
...
# Configuration
GITHUB_REPO="General-Brash/Personal_Sub2"
...
get_current_version() {
    if [ -f "$INSTALL_DIR/sub2api" ]; then
        ...
        "$INSTALL_DIR/sub2api" --version 2>/dev/null | grep -oE 'v?[0-9]+\.[0-9]+\.[0-9]+(-P[0-9]+|-personal\.[0-9]+)?' | head -1 || echo "unknown"
    else
        echo "not_installed"
    fi
}
...
Documentation=https://github.com/General-Brash/Personal_Sub2
...
upgrade() {
    ...
    # Get current version
    CURRENT_VERSION=$("$INSTALL_DIR/sub2api" --version 2>/dev/null | grep -oE 'v?[0-9]+\.[0-9]+\.[0-9]+(-P[0-9]+|-personal\.[0-9]+)?' | head -1 || echo "unknown")
