...
grep -q '^SUB2API_IMAGE_TAG=latest$' "${ENV_FILE}" || \
    fail "init did not retain the shared image tag default"
grep -q '^APPLE_CONTAINER_SUB2API_IMAGE=$' "${ENV_FILE}" || \
    fail "init did not leave the Apple image override empty"
