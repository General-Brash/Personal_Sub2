...
Personal_Sub2 includes a payment system that enables user self-service top-up without deploying a separate payment service.
...
> When selecting an EasyPay-compatible provider, verify its current API compatibility, settlement rules, fees, availability, security, and compliance requirements directly with the provider. This project does not guarantee or endorse any third-party payment provider.
...
If you previously used Sub2ApiPay as an external payment system, you can migrate to the built-in payment system:

### Key Differences

| Aspect | Sub2ApiPay | Built-in Payment |
|--------|-----------|-----------------|
| Deployment | Separate service (Next.js + PostgreSQL) | Built into Personal_Sub2, no extra deployment |
| Payment Methods | EasyPay, Alipay, WeChat, Stripe | Same |
| Configuration | Environment variables + separate admin UI | Unified in Personal_Sub2 admin dashboard |
| Top-up Integration | Via Admin API callback | Internal processing, more reliable |
| Subscription Plans | Supported | Supported |
| Order Management | Separate admin interface | Integrated in Personal_Sub2 admin dashboard |

### Migration Steps

1. Enable payment in the Personal_Sub2 admin dashboard and configure providers (use the same payment credentials)
2. Update webhook callback URLs to Personal_Sub2's callback endpoints
